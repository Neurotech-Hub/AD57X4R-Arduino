...
public:
  ...
  AD57X4R();
  AD57X4R(size_t chip_select_pin, double vref = 0, uint32_t spiClockSpeed = 1000000);
...
private:
  size_t chip_select_pin_ = UNINITIALIZED_PIN;
  size_t ldac_pin_;
  bool simultaneous_update_enabled_;
  size_t clr_pin_;
  double vref_;
  bool use_external_ref_;
  uint32_t spi_clock_speed_;
  ...
  // const static uint32_t SPI_CLOCK = 500000;
