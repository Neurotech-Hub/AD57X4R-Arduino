...
AD57X4R::AD57X4R(size_t chip_select_pin, double externalRef, uint32_t spiClockSpeed)
{
  initialize();
  setChipSelectPin(chip_select_pin);
  spi_clock_speed_ = spiClockSpeed;
  if (externalRef == 0)
  {
    use_external_ref_ = false;
    setVref(2.5); // Use default internal reference
  }
  else
  {
    use_external_ref_ = true;
    setVref(externalRef); // Use the provided external reference
  }
}

void AD57X4R::setVref(double vref)
{
  vref_ = vref;
}
...
double AD57X4R::getVoltageMin(size_t channel)
{
  ...
  case UNIPOLAR_10V:
  case UNIPOLAR_10V8:
    voltage_min = 0.0; // Unipolar outputs have a minimum of 0
    break;

  case BIPOLAR_5V:
    voltage_min = -(vref_ * 4.0) / 2; // Bipolar: Vmin = -(Vref * Gain) / 2
    break;

  case BIPOLAR_10V:
    voltage_min = -(vref_ * 8.0) / 2;
    break;

  case BIPOLAR_10V8:
    voltage_min = -(vref_ * 8.64) / 2;
    break;
...
double AD57X4R::getVoltageMax(size_t channel)
{
  ...
  case UNIPOLAR_5V:
    gain = 2.0;
    voltage_max = vref_ * gain; // Vout = Vref * Gain
    break;
  case UNIPOLAR_10V:
    gain = 4.0;
    voltage_max = vref_ * gain;
    break;
  case UNIPOLAR_10V8:
    gain = 4.32;
    voltage_max = vref_ * gain;
  ...
  case BIPOLAR_5V:
    gain = 4.0;
    voltage_max = (vref_ * gain) / 2; // Divide by 2 for bipolar
    break;
  case BIPOLAR_10V:
    gain = 8.0;
    voltage_max = (vref_ * gain) / 2; // Divide by 2 for bipolar
...
{
  if (chip_select_pin_ != UNINITIALIZED_PIN)
  {
    digitalWrite(chip_select_pin_, LOW);
  }
  else
  {
    // Handle the error, e.g., print a message or set a default action
    Serial.println("Error: chip select pin not initialized!");
  }
}
...
{
  enableChipSelect();
  SPI.beginTransaction(SPISettings(spi_clock_speed_, MSBFIRST, SPI_MODE2));
}
...
void AD57X4R::powerUpAllDacs()
{
  ...
  if (use_external_ref_)
  {
    data |= POWER_CONTROL_REF;
  }
